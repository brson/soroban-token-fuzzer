...
use soroswap_pair_token::soroswap_pair_token::{SoroswapPairToken, SoroswapPairTokenClient};
...
struct AdminClient<'a> {
    client: SoroswapPairTokenClient<'a>,
}
...
impl ContractTokenOps for TokenOps {
    ...
    fn register_contract_init(&self, env: &Env, admin: &Address) -> Address {
        let token_contract_id = env.register_contract(None, SoroswapPairToken);

        let admin_client = SoroswapPairTokenClient::new(&env, &token_contract_id);
    ...
    fn reregister_contract(&self, env: &Env, token_contract_id: &Address) {
        env.register_contract(Some(token_contract_id), SoroswapPairToken);
    }
    ...
    fn new_admin_client<'a>(
        ...
        Box::new(AdminClient {
            client: SoroswapPairTokenClient::new(&env, &token_contract_id),
        })
    }
}
